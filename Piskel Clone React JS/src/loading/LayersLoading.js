...
function getValidPixel ({ images, x, y, imgIndex, size, activeLayer }) {
    ...
    if (images[imgIndex] === undefined || imgIndex === activeLayer || images[imgIndex].data[pixelNum + 3] === 0) {
        getValidPixel({ images, x, y, imgIndex: imgIndex + 1, size, activeLayer });
        return;
    }
...
export default class LayersLoading {
    addNewLayer () {
        ...
        this.setState({
            layers,
            frames
        }, () => this.layersLoading.setActiveLayer.bind(this)(this.state.activeLayer));
    }
        ...
        this.setState({ activeLayer: n }, () => {
            ...
            for (let y = 0; y < this.state.canvasSize; y += 1) {
                for (let x = 0; x < this.state.canvasSize; x += 1) {
                    getValidPixel({
                        images: this.state.frames[this.state.activeFrame].img,
                        x,
                        y,
                        imgIndex: 0,
