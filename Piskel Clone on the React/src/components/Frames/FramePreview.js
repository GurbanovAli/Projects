...
export default class FramePreview extends Component {
    ...
    render () {
        ...
        return (
            <>
                {(this.props.proxyFrame === -1 && this.props.number === 0) &&
                ...
                >
                    <canvas
                        style={canvasScale}
                        className="frames-bar__frame-preview-canvas"
                        width={this.props.canvasSize}
                        height={this.props.canvasSize}
                    />
                    <div className="frames-bar__frame-preview-num">{this.props.number}</div>
                    <button
                        className="frames-bar__frame-preview-btn delete-btn"
                        onClick={this.deleteFrame}
                    />
                    <button
                        className="frames-bar__frame-preview-btn dublicate-btn"
                        onClick={this.props.dublicateFrame}
                    />
                </div>
                {(this.props.proxyFrame === this.props.number) && <div className="proxy-frame" />}
            </>
        );
    }
}
