...
export default class Layers extends Component {
    ...
    setEditMode (layerNum) {
        if (layerNum >= 0) {
            this.setState({ onEdit: layerNum },
                () => document.getElementById(`layer-name-input-${layerNum}`).focus());
        } else {
            this.setState({ onEdit: layerNum });
        }
    }
        ...
        return (
            <div className="layers-wrapper">
                ...
                <div className="layers-tools">
                    <button
                        className="layers-tools__add-new-layer-btn"
                        onClick={this.props.addNewLayer.bind(this)}
                    >
                        +
                    </button>
                </div>
